...
def main():
    data_path = "/tmp/ibis-profiling/data_20M_20cols.parquet"
    report_path = "/tmp/ibis-profiling/report_20M_20cols.html"
    ...
    # Force DuckDB to be less aggressive with memory
    ...
    print("--- Profiling 20M rows x 20 columns (Minimal: False) ---")
    try:
        report = ProfileReport(
            table,
            minimal=False,
            parallel=False,
            global_batch_size=5,  # Tiny batches for maximum safety
            on_progress=on_progress,
            title="20M Rows x 20 Columns Performance Test",
        )
